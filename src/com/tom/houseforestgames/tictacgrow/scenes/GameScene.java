...
public abstract class GameScene extends Scene implements ITouchArea
{
	...
	@Override public float[] convertSceneToLocalCoordinates(float pX, float pY)
	{
		return new float[] { pX, pY };
	}

	//scenes should always be full-screen so no conversion neccessary
	@Override public float[] convertLocalToSceneCoordinates(float pX, float pY)
	{
		return new float[] { pX, pY };
	}
	...
	public boolean wasTouched(int x0, int y0, int w, int h)
	{
		return (mTouchEnabled && mTouched && x0 <= mTouchX && mTouchX <= x0 + w && y0 <= mTouchY && mTouchY <= y0 + h);
	}

	//was touch in rectangle area released
	public boolean wasTouchReleased(int x0, int y0, int w, int h)
	{
		return (mTouchEnabled && mReleased && x0 <= mReleaseX && mReleaseX <= x0 + w && y0 <= mReleaseY && mReleaseY <= y0 + h);
	}
